...
class List {
	...
	public:
		...
		// Moves the current pointer/marker to specified index. If the index is out of bounds then exception is thrown
		void moveTo(size_t);
...
template <typename T>
size_t List<T>::getCurrentIndex() const {
	return current;
}
...
void List<T>::remove() {
	...
	if (list == nullptr) throw std::runtime_error("The list is empty! Cannot remove element!");
...
template <typename T>
void List<T>::update(const T& newValue) {
	if (current == -1) throw std::runtime_error("The list don't have any elements! Can't update current element!");
...
template <typename T>
void List<T>::moveTo(size_t index) {
	if (index < 0 || index > size-1) throw std::out_of_range("The given index is out of range!");
	current = index;
}
...
template <typename T>
bool List<T>::find(const T& elem) {	

	for (int i = 0; i < size; i++) {
		if (list[i] == elem) {
			// If the element is present, then simply move the current pointer/marker to this index.
			current = i;
			return true;
		}
	}
...
const T& List<T>::operator[] (size_t index) const {
	if (index < 0 || index > size-1) throw std::out_of_range("Invalid index is given!");
	return list[index];
}
